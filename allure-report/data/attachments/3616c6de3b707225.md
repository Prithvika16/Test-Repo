...
- Name: Popups Demo Playwright
- Location: C:\PlaywrightDemo\tests\PracticeTests\Alerts\popups.spec.js:2:1
...
Call log:
  - waiting for locator('.cart-full .cart-item') to be visible

    at C:\PlaywrightDemo\tests\PracticeTests\Alerts\popups.spec.js:18:16
```
...
- link "thumbnail of Dragon Energy Potion Dragon Energy Potion $4.99":
  ...
  - text: $4.99
- link "thumbnail of Hiking Boots for Outdoor Adventures Hiking Boots for Outdoor Adventures $89.99":
  - /url: https://web-scraping.dev/product/7
  - img "thumbnail of Hiking Boots for Outdoor Adventures"
  - heading "Hiking Boots for Outdoor Adventures" [level=3]
  - text: $89.99
- link "thumbnail of Dark Red Energy Potion Dark Red Energy Potion $4.99":
  ...
  - text: $4.99
- link "thumbnail of Classic Leather Sneakers Classic Leather Sneakers $79.99":
  - /url: https://web-scraping.dev/product/11
  - img "thumbnail of Classic Leather Sneakers"
  - heading "Classic Leather Sneakers" [level=3]
  - text: $79.99
- contentinfo:
...
```ts
   ...
   2 | test('Popups Demo Playwright', async({page}) => {
   3 |     page.on('dialog', async dialog =>{
   4 |         if(dialog.message().includes('clear your cart')){
   5 |             console.log('clicking "OK" to ${dialog.message()}');
   6 |             await dialog.accept(); //press "Yes"
   7 |         } else{
   8 |             await dialog.dismiss(); //press "No"
   9 |         }
  10 |     });
  11 |
  12 |     //Add something to Cart
  13 |     await page.goto('https://web-scraping.dev/product/1');
  14 |     await page.click('.add-to-cart');
  15 |
  16 |     //Clear out the cart to trigger the popup event
  17 |     await page.goto('https://web-scraping.dev/product/1');
> 18 |     await page.waitForSelector('.cart-full .cart-item');
     |                ^ Error: page.waitForSelector: Test timeout of 30000ms exceeded.
  19 |     await page.click("//button[contains(text(),'Clear')])[2]");
  20 |
  21 |     //verify the cart is clear
  22 |     const cartItem = await page.$('.cart-item .cart-title');
  23 |     console.log('items in cart: ${cartItem}'); //should be null
  24 |
  25 | });
  26 |
```